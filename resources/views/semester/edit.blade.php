@foreach ($tahunSemester as $item)
    <x-modal name="edit-modal-{{ $item['id'] }}" title="Edit Semester" maxWidth="2xl">
        <form action="{{ role_route('semester.update', ['tahun_semester' => $item['id']]) }}" method="POST"
            ...
            {{-- Mulai --}}
            <x-form.date-picker name="mulai" label="Tanggal Mulai" :value="old('mulai', $item['semester_mulai'])" required />

            {{-- Selesai --}}
            <x-form.date-picker name="selesai" label="Tanggal Selesai" :value="old('selesai', $item['semester_selesai'])" required />
