<x-modal name="form-create-semester" title="Tambah Semester" maxWidth="2xl">
    <form action="{{ role_route('semester.store') }}" method="POST" enctype="multipart/form-data"
        class="space-y-6 sm:p-6">
        @csrf
        <input type="hidden" name="tab" value="tahun-semester">

        {{-- Tahun --}}
        {{-- <x-form.input name="tahun" label="Tahun Ajaran" required /> --}}
        <x-form.select name="tahun_ajaran_id" label="Tahun Ajaran" :options="$tahunOptions" :selected="old('tahun_ajaran_id', '')"
            required/>
        {{-- Semester --}}
        <x-form.select name="semester" label="Semester" :options="['Ganjil' => 'Ganjil', 'Genap' => 'Genap']" placeholder="Pilih semester" required />

            {{-- Mulai --}}
            <x-form.date-picker name="mulai" label="Tanggal Mulai" :value="old('mulai', '')" required />

            {{-- Selesai --}}
            <x-form.date-picker name="selesai" label="Tanggal Selesai" :value="old('selesai', '')" required />
