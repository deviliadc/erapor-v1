...
@php
    $showToolbar = (
        (is_array($filters) && count($filters) > 0) ||
        $enableAddButton ||
        $enableImport ||
        $enableExport
    );
...
@if ($showToolbar)
<div class="border-b border-gray-200 py-4 dark:border-gray-800 mb-4 flex flex-col gap-3 px-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
    {{-- <div class="flex flex-wrap items-center gap-4"> --}}
    <div class="flex flex-wrap items-center gap-4 overflow-x-auto max-w-full scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent">
        @foreach ($filters as $filter)
            <x-select-filter :name="$filter['name']" :label="$filter['label']" :options="$filter['options']" :value-key="$filter['valueKey'] ?? 'id'" :label-key="$filter['labelKey'] ?? 'name'"
    ...
    <div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
        ...
        @if ($enableImport)
            ...
            <x-table.dropdown label="Import">
                <a href="#" class="dropdown-item flex items-center gap-2">
                    ...
                    Download Template</a>
                <a href="#" class="dropdown-item flex items-center gap-2">Import File</a>
            </x-table.dropdown>
        @endif
        ...
        @if ($enableExport)
            ...
            <x-table.dropdown label="Export">
                <a href="{{ $route }}?type=excel&filename={{ $filename }}"
