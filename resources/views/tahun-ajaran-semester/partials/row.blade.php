...
<td class="px-4 py-3 text-gray-700 dark:text-gray-400 whitespace-nowrap">{{ $item['is_active'] }}</td>
<td class="px-4 py-3 text-gray-700 dark:text-gray-400 whitespace-nowrap">
    {{-- Actions --}}
    <div class="flex gap-2 items-center">
        {{-- Delete --}}
        <form action="{{ route('admin.tahun_semester.destroy', $item['id']) }}" method="POST"
        ...
        {{-- Edit --}}
        <button onclick="window.location.href='{{ route('admin.tahun_semester.edit', ['tahun_semester' => $item['id']]) }}'"
            class="text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white/90">
            <svg class="fill-current" width="21" height="21" viewBox="0 0 21 21" fill="none"
                xmlns="http://www.w3.org/2000/svg">
                <path fill-rule="evenodd" clip-rule="evenodd"
