...
<x-app-layout>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <x-table.toolbar :filters="$filters" :enable-add-button="true" :enable-search="true" :enable-import="false" :enable-export="false">
            <x-slot name="addButton">
                <form action="{{ role_route('validasi_semester.validateAll') }}" method="POST" class="inline">
                    @csrf
                     <input type="hidden" name="semester_id" value="{{ $semesterId }}">
                    {{-- <input type="hidden" name="kelas" value="{{ request('kelas') }}"> --}}
                    {{-- <input type="hidden" name="tahun_semester" value="{{ request('tahun_semester') }}"> --}}
        ...
        <x-table :columns="[
            ...
            :selectable="false"
            row-view="validasi-semester.partials.row"
