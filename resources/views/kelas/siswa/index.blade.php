@php
    $filters = [
        [
            ...
            'value' => request('tahun_semester_filter') ?? ($tahunSemesterId ?? $tahunAktif?->id),
        ],
    ];
@endphp
...
<x-app-layout>
    ...
    @if ($tahunAktif)
        <div class="mb-4 text-base font-semibold text-brand-600 dark:text-white">
            Tahun Ajaran Aktif: {{ $tahunAktif->tahun }} ({{ $tahunAktif->semester }})
        </div>
    @endif
        ...
        <x-table.toolbar :filters="$filters" :enable-add-button="true" :enable-import="false" :enable-export="false" :enable-search="true"
            ...
            <x-slot name="addButton">
                <div class="flex gap-2">
                    {{-- Tombol Generate --}}
                    {{-- <a href="{{ route('kelas.generate.absen', ['kelas_id' => $kelas->id, 'tahun_semester_filter' => request('tahun_semester_filter') ?? ($tahunSemesterId ?? $tahunAktif?->id)]) }}"
    ...
    <x-modal name="form-generate-absen" title="Generate Nomor Absen" maxWidth="md">
        <form
            ...
            method="GET">
            @csrf
            <input type="hidden" name="tahun_semester_filter" value="{{ request('tahun_semester_filter') ?? ($tahunSemesterId ?? $tahunAktif?->id) }}">
            <div class="p-6">
