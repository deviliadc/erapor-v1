...
<x-app-layout>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <x-table.toolbar
            ...
            :enable-import="auth()->user()->hasRole('admin') || auth()->user()->hasRole('guru')" importModalName="import-siswa" {{-- :enable-import="false" --}}
            :enable-export="true" :enable-search="true" :route="route($routePrefix . 'siswa.index')" :route-create="route($routePrefix . 'siswa.create')" :routeTemplate="route('export.excel', ['type' => 'template_siswa'])" />

        <!-- Tombol buka modal -->
        <button @click="$dispatch('open-modal', 'import-siswa')"
        ...
        ]" :data="$siswa" :total-count="$totalCount" row-view="siswa.partials.row"
            :actions="[
                'detail' => true,
                'edit' => true,
                'delete' => !$isGuru,
                'routes' => [
                    ...
                    'delete' => n($item) => role_route('siswa.destroy', ['siswa' => $item['id']]),
                ]
            ]" />
    </div>
</x-app-layout>
