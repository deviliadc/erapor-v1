<x-app-layout>
    ...
    <div x-data="{
        ...
        <form method="GET" class="mb-4 flex flex-wrap gap-4">
            <x-form.select label="Tahun Semester" name="tahun_semester_id" {{-- :options="$daftarTahunSemester->mapWithKeys(fn($ts) => [
                ...
                :selected="request('tahun_semester_id', $tahunAktif->id)" placeholder="Pilih Tahun Semester" searchable required onchange="this.form.submit()" />
                                ...
                                <table class="min-w-[1200px] w-full text-sm text-center table-auto whitespace-nowrap">
                                    ...
                                    <tbody class="bg-white dark:bg-gray-900">
                                                    ...
                                                    @foreach ($tpList as $tp)
                                                        ...
                                                        @foreach ($tpList as $tp)
                                                            @php
                                                                $val =
                                                                    old(
                                                                        "nilai.{$gk->mapel->id}.{$ksId}.formatif_{$tp->id}",
                                                                    ) ??
                                                                    ($nilaiMapelTab['formatif'][$ksId][$tp->id] ??
                                                                        null);
                                                            @endphp
                                                            <td>
                                                                <input x-show="editMode" type="number"
                                                                    name="nilai[{{ $gk->mapel->id }}][{{ $ksId }}][formatif_{{ $tp->id }}]"
                                                                    value="{{ $val }}"
                                                                    class="input-nilai w-16 text-sm border rounded" />
                                                                <span x-show="!editMode">
                                                                    {{ is_null($val) || $val === '' ? '-' : $val }}
                                                ...
                                                @if ($lmList->isNotEmpty())
                                                    @foreach ($lmList as $bab)
                                                        ...
                                                        @php
                                                            ...
                                                            $val =
                                                                old(
                                                                    "nilai.{$gk->mapel->id}.{$ksId}.sumatif_{$babKey}",
                                                                ) ??
                                                                ($nilaiMapelTab['sumatif'][$ksId][$babKey] ?? null);
                                                        @endphp
                                                        <td>
                                                            <input x-show="editMode" type="number"
                                                                name="nilai[{{ $gk->mapel->id }}][{{ $ksId }}][sumatif_{{ $babKey }}]"
                                                                value="{{ $val }}"
                                                                class="input-nilai w-16 text-sm border rounded" />
                                                            <span x-show="!editMode">
                                                                {{ is_null($val) || $val === '' ? '-' : $val }}
                                        ...
                                        <td>
                                            @if ($periode == 'tengah')
                                                <input x-show="editMode" type="number"
                                                    name="nilai[{{ $gk->mapel->id }}][{{ $ksId }}][uts_nontes]"
                                                    value="{{ old("nilai.{$gk->mapel->id}.{$ksId}.uts_nontes") ?? ($nilaiMapelTab['uts'][$ksId]['non_tes'] ?? '') }}"
                                                    class="input-nilai w-16 text-sm border rounded" />
                                                <span x-show="!editMode">
                                                    {{ $nilaiMapelTab['uts'][$ksId]['non_tes'] ?? '-' }}
                                                </span>
                                        ...
                                        <td>
                                            @if ($periode == 'tengah')
                                                <input x-show="editMode" type="number"
                                                    name="nilai[{{ $gk->mapel->id }}][{{ $ksId }}][uts_tes]"
                                                    value="{{ old("nilai.{$gk->mapel->id}.{$ksId}.uts_tes") ?? ($nilaiMapelTab['uts'][$ksId]['tes'] ?? '') }}"
                                                    class="input-nilai w-16 text-sm border rounded" />
                                                <span x-show="!editMode">
                                                    {{ $nilaiMapelTab['uts'][$ksId]['tes'] ?? '-' }}
                                                </span>
                                        ...
                                        @if ($periode == 'akhir')
                                            ...
                                            <td>
                                                <input x-show="editMode" type="number"
                                                    name="nilai[{{ $gk->mapel->id }}][{{ $ksId }}][uas_nontes]"
                                                    value="{{ old("nilai.{$gk->mapel->id}.{$ksId}.uas_nontes") ?? ($nilaiMapelTab['uas'][$ksId]['non_tes'] ?? '') }}"
                                                    class="input-nilai w-16 text-sm border rounded" />
                                                <span x-show="!editMode">
                                                    {{ $nilaiMapelTab['uas'][$ksId]['non_tes'] ?? '-' }}
                                                </span>
                                            ...
                                            <td>
                                                <input x-show="editMode" type="number"
                                                    name="nilai[{{ $gk->mapel->id }}][{{ $ksId }}][uas_tes]"
                                                    value="{{ old("nilai.{$gk->mapel->id}.{$ksId}.uas_tes") ?? ($nilaiMapelTab['uas'][$ksId]['tes'] ?? '') }}"
                                                    class="input-nilai w-16 text-sm border rounded" />
                                                <span x-show="!editMode">
                                                    {{ $nilaiMapelTab['uas'][$ksId]['tes'] ?? '-' }}
                                                </span>
