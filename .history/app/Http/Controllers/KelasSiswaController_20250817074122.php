...
class KelasSiswaController extends Controller
{
    ...
    public function index(Request $request, string $kelas)
    {
        ...
        // Ambil semua tahun semester untuk select filter (terbaru di atas)
        $tahunSemesterCollection = TahunSemester::with('tahunAjaran')
            ->orderByDesc(
        ...
        $tahunSemesterSelect = $tahunSemesterCollection->map(function ($item) use ($tahunAktif) {
            return [
                'id' => $item->id,
                'name' => ($item->tahunAjaran ? $item->tahunAjaran->tahun : '-') . ' - ' . ucfirst($item->semester) . ($item->id == $tahunAktif?->id ? ' (Tahun Aktif)' : ''),
            ];
        });

        // Untuk select tahun semester di form promote (hilangkan tahun yang sedang digunakan/dibuka, terbaru di atas)
        $tahunSemesterList = $tahunSemesterCollection->where('id', '!=', $tahunSemesterId)
            ->map(function ($ts) use ($tahunAktif) {
                return [
                    'id' => $ts->id,
                    'name' => ($ts->tahunAjaran ? $ts->tahunAjaran->tahun : '-') . ' - ' . ucfirst($ts->semester) . ($ts->id == $tahunAktif?->id ? ' (Tahun Aktif)' : ''),
                ];
            })
            ->values();
