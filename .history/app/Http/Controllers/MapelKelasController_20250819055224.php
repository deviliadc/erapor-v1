...
class MapelKelasController extends Controller
{
    ...
    public function show(Request $request, string $id)
    {
        ...
        //     ];
        // });
        $mapelList->getCollection()->transform(function ($item) use ($mapelTerpakaiAll, $kelas, $tahunAjaranId) {
            // Mapel yang sudah dipakai di kelas ini, kecuali yang sedang diedit
            $mapelTerpakai = array_diff($mapelTerpakaiAll, [$item->mapel_id]);

            // Mapel yang belum terpakai di kelas ini (tahun ajaran ini)
            $mapelBelumTerpakai = Mapel::whereNotIn('id', $mapelTerpakai)
                ->pluck('nama', 'id')
                ->toArray();

            // Pastikan mapel yang sedang diedit tetap muncul di options
            if ($item->mapel_id && !isset($mapelBelumTerpakai[$item->mapel_id])) {
                $mapelBelumTerpakai[$item->mapel_id] = $item->mapel->nama ?? '-';
            }

            // Urutkan agar mapel yang sedang diedit tetap di atas (optional)
            $mapelOptions = collect($mapelBelumTerpakai)
                ->sortKeys()
                ->toArray();
