...
public function store(Request $request)
{
    ...
    if ($isActive) {
        // Nonaktifkan semua tahun ajaran & semester lain
    ...
    //     // Cek duplikasi tahun ajaran selain data sendiri
    ...
    public function update(Request $request, string $id)
{
    ...
    if ($isActive) {
        ...
        // Ambil semester terkait (utamakan Ganjil)
        $semesterGanjil = $item->tahunSemester()->where('semester', 'Ganjil')->first();
        if ($semesterGanjil) {
            $semesterGanjil->update(['is_active' => true]);
        } else {
            // Kalau tidak ada semester, semua semester tetap nonaktif
        }
    } else {
    ...
    public function destroy(string $id)
    {
        ...
        // Cek apakah ada tahun semester terkait
        $tahunSemesterCount = $item->tahunSemester()->count();
        if ($tahunSemesterCount > 0) {
            return redirect()->to(role_route('tahun-semester.index', ['tab' => request()->tab ?? 'tahun-ajaran']))
                ->with('error', 'Tidak bisa hapus Tahun Ajaran karena masih ada Tahun Semester terkait.');
        }

        // Cek relasi lain (misal p5_proyek)
        $relatedProyek = DB::table('p5_proyek')
            ->whereIn('tahun_semester_id', $item->tahunSemester()->pluck('id'))
            ->count();
        if ($relatedProyek > 0) {
            return redirect()->to(role_route('tahun-semester.index', ['tab' => request()->tab ?? 'tahun-ajaran']))
                ->with('error', 'Tidak bisa hapus Tahun Ajaran karena masih ada data proyek terkait.');
        }
