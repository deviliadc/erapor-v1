...
class RaporController extends Controller
{
    ...
    public function cetakP5(Request $request, Siswa $siswa)
    {
        ...
        $kelasSiswa = $siswa->kelasSiswa()
            ->whereHas('kelas')
            ->whereHas('tahunSemester', fn($q) => $q->where('id', $tahunSemesterId))
            ->first();
        ...
        $proyek = $siswa->p5()
            ...
            ->with(['capaian', 'dimensi', 'elemen'])
            ->first();

        // Jika data proyek tidak ada, tampilkan halaman kosong atau pesan
        if (!$proyek) {
            return view('rapor.p5', compact('siswa', 'kelas', 'pengaturan', 'guruKelas'))->with('error', 'Data P5 tidak ditemukan.');
        }

        return view('rapor.p5', compact('siswa', 'kelas', 'pengaturan', 'proyek', 'guruKelas'));
    }
