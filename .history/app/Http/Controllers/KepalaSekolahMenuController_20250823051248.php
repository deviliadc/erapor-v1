...
public function rekapPresensiDetail(Request $request, $kelas_id)
{
   ...
   $waliKelas = GuruKelas::where('kelas_id', $kelas_id)
    ->where('tahun_ajaran_id', $tahunAjaranId)
    ->where('peran', 'wali')
    ->with('guru') // relasi ke model Guru
    ->first()?->guru?->nama ?? '-';
    ...
    foreach ($kelasSiswaList as $ks) {
        // Ambil presensi berdasarkan kelas_siswa_id
        $presensi = PresensiDetail::where('kelas_siswa_id', $ks->id)
            ->whereHas('presensiHarian', function ($q) use ($tahunAjaranId) {
                $q->whereHas('tahunSemester', function ($q2) use ($tahunAjaranId) {
