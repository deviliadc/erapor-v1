...
class TahunSemesterController extends Controller
{
    ...
    {
        $query = TahunAjaran::orderByDesc('mulai'); // urut dari tahun terbaru
        $totalCount = $query->count();
        $paginator = $query->paginate($perPage)->withQueryString();
    ...
    {
        $query = TahunSemester::orderByDesc('mulai'); // urut dari semester terbaru
        $totalCount = $query->count();
        $paginator = $query->paginate($perPage)->withQueryString();
        ...
        $tahunOptions = TahunAjaran::orderByDesc('mulai')->pluck('tahun', 'id')->toArray();
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->to(role_route('tahun-semester.index', ['tab' => $request->tab ?? 'tahun-semester']))->with('success', 'Data berhasil ditambahkan.');
    }
    ...
    if ($tahunAjaranBerubah || $semesterBerubah) {
        ...
        // Cek duplikat tahun ajaran dan semester (kecuali data yang sedang diedit)
        $duplikat = TahunSemester::where('tahun_ajaran_id', $validated['tahun_ajaran_id'])
            ->where('semester', $validated['semester'])
            ->where('id', '!=', $id)
            ->exists();
    ...
    public function destroy(string $id)
    {
        $item = TahunSemester::findOrFail($id);
        if ($item->is_active) {
            return redirect()->to(role_route('tahun-semester.index', ['tab' => $request->tab ?? 'tahun-semester']))
