...
class SiswaController extends Controller
{
    ...
    public function update(Request $request, string $id)
    {
        ...
        // Ambil tahun ajaran aktif dari tahun_semester
        $tahunSemesterAktif = TahunSemester::where('is_active', 1)->first();
        if ($tahunSemesterAktif && !empty($validated['kelas_id'])) {
            $tahunAjaranId = $tahunSemesterAktif->tahun_ajaran_id;
    ...
    public function import(Request $request)
    {
        ...
        Excel::import(
            new ReusableImport(function ($row, $index) use (&$success, &$failed, &$errors, &$headerError, $requiredHeaders) {
                ...
                // Hilangkan tanda petik satu di awal jika ada
                if ($data['nipd'] && str_starts_with($data['nipd'], "'")) {
                    $data['nipd'] = ltrim($data['nipd'], "'");
                }
                if ($data['nisn'] && str_starts_with($data['nisn'], "'")) {
                    $data['nisn'] = ltrim($data['nisn'], "'");
                }
                ...
                if ($validator->fails()) {
                    $failed++;
                    $errMsg = ke($data['nama'] ?? '(Tanpa Nama)') . ' error: ' . implode(', ', $validator->errors()->all());
                    $errors[] = $errMsg;
                    return;
                }
                ...
                $siswaBaru = Siswa::where('nisn', $data['nisn'])->first();
                $tahunSemesterAktif = TahunSemester::where('is_active', 1)->first();
                if ($siswaBaru && $tahunSemesterAktif && !empty($data['kelas'])) {
                    $kelasObj = Kelas::where('nama', $data['kelas'])->first();
                    if ($kelasObj) {
                        $tahunAjaranId = $tahunSemesterAktif->tahun_ajaran_id;
                        // Cek apakah sudah ada data kelas siswa di tahun ajaran aktif
                        $sudahAda = KelasSiswa::where('siswa_id', $siswaBaru->id)
                            ->where('tahun_ajaran_id', $tahunAjaranId)
                            ->where('kelas_id', $kelasObj->id)
                            ->exists();
                        ...
                        if ($sudahAda) {
                            $failed++;
                            $errors[] = "{$data['nama']} sudah terdaftar di kelas {$kelasObj->nama} tahun {$tahunSemesterAktif->tahun} semester {$tahunSemesterAktif->semester}";
                        } else {
                            KelasSiswa::create([
                                'siswa_id' => $siswaBaru->id,
                                'tahun_ajaran_id' => $tahunAjaranId,
