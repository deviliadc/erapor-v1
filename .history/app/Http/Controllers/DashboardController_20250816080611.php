...
class DashboardController extends Controller
{
    ...
    protected function adminDashboard()
    {
        ...
        $chartSiswa = collect($tahunAjaranTerbaru)->map(function ($ta) use ($tahunSemester) {
            $semesterIds = $tahunSemester->where('tahun_ajaran_id', $ta->id)->pluck('id');
            $total = Siswa::whereHas('kelasSiswa', function ($q) use ($semesterIds) {
                $q->whereIn('tahun_semester_id', $semesterIds)
                    ->where('status', 'Aktif');
            })->count();
            return [
                'label' => $ta->tahun,
