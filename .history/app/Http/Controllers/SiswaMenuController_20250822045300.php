...
class SiswaMenuController extends Controller
{
    ...
    public function nilaiMapel()
    {
        ...
        // ambil semua daftar tahun semester untuk filter
        $daftarTahunSemester = TahunSemester::all();

        // ambil nilai mapel berdasarkan filter
        $tahunSemesterId = request('tahun_semester_id', $tahunAktif?->id);

         // Ambil semua nilai mapel untuk semester terpilih
    $nilaiMapel = NilaiMapel::where('kelas_siswa_id', $siswa->kelasSiswaAktif()?->id)
        ->where('tahun_semester_id', $tahunSemesterId)
        ->get()
        ->groupBy('mapel_id');
