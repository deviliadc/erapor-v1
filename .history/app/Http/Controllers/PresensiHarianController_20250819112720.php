    ...
    public function import(Request $request)
    {
        ...
        try {
            ...
            foreach ($imported as $sheetIndex => $rows) {
                ...
                // Nama kelas dari sheet (gunakan nama tab di template export)
                $sheetName = $request->file('file')->getClientOriginalName(); // fallback jika tidak dapat nama tab
                // Jika library mendukung, bisa ambil nama tab dari $imported key

                // Heading
                $headings = array_map('trim', $rows[0]);
                if (!$expectedHeadings) {
                    $expectedHeadings = ['No', 'NIS', 'Nama Siswa'];
                    // Tambahan tanggal-tanggal
                    for ($i = 3; $i < count($headings); $i++) {
                        $expectedHeadings[] = $headings[$i];
                    }
                }
                if ($headings !== $expectedHeadings) {
                    $errors[] = "Sheet " . ($sheetName ?? $sheetIndex + 1) . ": Format kolom tidak sesuai template.";
                    continue;
                }
                ...
                foreach (array_slice($rows, 1) as $rowIdx => $row) {
                    ...
                    $siswa = \App\Models\Siswa::where('nama', $namaSiswa)
                        ...
                        ->first();
                    if (!$siswa) {
                        $errors[] = "Sheet " . ($sheetName ?? $sheetIndex + 1) . " baris " . ($rowIdx + 2) . ": Siswa '$namaSiswa' (NIPD: $nipd) tidak ditemukan.";
                        continue;
                    }

                    // Validasi kelas_siswa di tahun ajaran aktif
                    $tahunAjaranAktif = \App\Models\TahunAjaran::where('is_active', true)->first();
                    $kelasSiswa = \App\Models\KelasSiswa::where('siswa_id', $siswa->id)
                        ...
                        ->first();
                    if (!$kelasSiswa) {
                        $errors[] = "Sheet " . ($sheetName ?? $sheetIndex + 1) . " baris " . ($rowIdx + 2) . ": Siswa '$namaSiswa' tidak terdaftar di kelas pada tahun ajaran aktif.";
                        continue;
                    }
                    ...
                    for ($col = 3; $col < count($row); $col++) {
                        ...
                        $statusValid = ['H', 'S', 'I', 'A'];
                        if (!in_array($status, $statusValid)) {
                            $errors[] = "Sheet " . ($sheetName ?? $sheetIndex + 1) . " baris " . ($rowIdx + 2) . " tanggal $tanggal: Status '$status' tidak valid.";
                            continue;
                        }
