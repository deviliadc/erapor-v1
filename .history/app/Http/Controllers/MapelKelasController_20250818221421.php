    ...
    public function index(Request $request)
{
    ...
    // Query guru_kelas + mapel + guru
    $query = GuruKelas::with(['mapel', 'guru', 'kelas'])
        ->where('tahun_ajaran_id', $tahunAjaranId)
        ->where('peran', 'pengajar');

    $totalCount = $query->count();

    // Paginasi + transformasi
    $paginator = $query->paginate($perPage)->withQueryString();

    $mapelList = $paginator->through(function ($item) {
        return [
            'id'         => $item->id,
    ...
    public function store(Request $request, string $kelas)
    {
        ...
        // return redirect()->to(role_route('mapel-kelas.index', $kelas))
    ...
    public function show(Request $request, string $id)
    {
        ...
        $perPage = $request->input('per_page', 10); // Semua mapel yang sudah dipakai di kelas ini (peran pengajar)
        $mapelTerpakaiAll = GuruKelas::where('kelas_id', $kelas->id)
        ...
        $title = 'Mapel Kelas ' . $kelas->nama;
        $breadcrumbs = [
            ['label' => 'Manage Kelas', 'url' => role_route('mapel-kelas.index')],
            ['label' => 'Mapel'],
        ];
    ...
    public function update(Request $request, string $kelas, string $mapel)
    {
        ...
        return redirect()->to(role_route('mapel-kelas.index', [$kelas]))->with('success', 'Mapel berhasil diperbarui.');
    }
    ...
    public function destroy(string $kelas, string $id)
    {
        ...
        return redirect()->to(role_route('mapel-kelas.index', [$kelas]))->with('success', 'Mapel berhasil dihapus.');
    }
}
