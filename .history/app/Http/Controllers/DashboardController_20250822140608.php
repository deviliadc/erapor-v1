...
class DashboardController extends Controller
{
    ...
    protected function adminDashboard()
    {
        ...
        $totalLulusan = 0;
if ($tahunAjaranAktif) {
    $totalLulusan = Siswa::whereHas('kelasSiswa', function ($query) use ($tahunAjaranSekarang) {
        $query->where('status', 'Lulus')
              ->whereHas('tahunAjaran', function ($q) use ($tahunAjaranSekarang) {
                  $q->where('tahun', '<=', $tahunAjaranSekarang->tahun);
              });
    })->count();
}
