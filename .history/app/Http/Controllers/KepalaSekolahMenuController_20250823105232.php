...
class KepalaSekolahMenuController extends Controller
{
    ...
    public function nilaiMapel(Request $request)
    {
        ...
        return view('menu-kepsek.rekap-nilai-mapel', [
            ...
            'tahunSemesterSelect' => TahunSemester::with('tahunAjaran')
                ->join('tahun_ajaran', 'tahun_ajaran.id', '=', 'tahun_semester.tahun_ajaran_id')
                ->orderBy('tahun_ajaran.tahun', 'desc')
                ->ohprderByRaw("FIELD(semester, 'ganjil', 'genap')")
                ->select('tahun_semester.*')
                ->get()
                ->map(fn($ts) => [
                    'id' => $ts->id,
    ...
    public function nilaiMapelDetail(Request $request, $kelas_id)
    {
        ...
        $mapelOptions = GuruKelas::with('mapel')
            ->where('kelas_id', $kelas_id)
            ->where('tahun_ajaran_id', $tahunAjaranId)
            ->get()
            ->pluck('mapel')
        ...
        $mapelAktif = $mapelId
            ...
            : ($mapelOptions->first() ? \App\Models\Mapel::find($mapelOptions->first()['id']) : null);
        ...
        $nilaiMapel = [];
        foreach ($kelasSiswaList as $ks) {
            // Jika tidak ada mapel, $nilai = null
            $nilai = $mapelAktif
        ...
        return view('menu-kepsek.rekap-nilai-mapel-detail', [
            ...
            'breadcrumbs' => [
                ['label' => 'Rekap Nilai Mata Pelajaran',  'url' => route('kepala-sekolah.rekap-nilai-mapel.index')],
...
//     $mapelAktif = $mapelId ? \App\Models\Mapel::find($mapelId) : ($mapelOptions->first() ? \App\Models\Mapel::find($mapelOptions->first()['id']) : null);
...
//     $nilaiMapel = [];
//     foreach ($kelasSiswaList as $ks) {
//         $nilai = \App\Models\NilaiMapel::where('kelas_siswa_id', $ks->id)
//             ->where('tahun_semester_id', $tahunSemesterId)
