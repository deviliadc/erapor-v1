    ...
    <div class="max-w-xl mx-auto mt-8 rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
        <form action="{{ role_route('pengaturan-rapor.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="flex justify-end">
                ...
                {{-- filepath: resources/views/pengaturan-rapor/urutan-mapel.blade.php --}}
        <form action="{{ role_route('pengaturan-rapor.urutan-mapel.simpan') }}" method="POST">
            @csrf
            <table class="table table-bordered">
                <thead>
                    <tr>
                        <th>Mapel</th>
                        <th>Urutan di Rapor</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($mapelList as $mapel)
                        <tr>
                            <td>{{ $mapel->nama }}</td>
                            <td>
                                <input type="number" name="urutan[{{ $mapel->id }}]" value="{{ $mapel->urutan ?? '' }}" min="1" class="form-control" style="width:80px;">
                            </td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
            <button type="submit" class="btn btn-primary mt-3">Simpan Urutan</button>
        </form>
    </div>
</x-app-layout>
