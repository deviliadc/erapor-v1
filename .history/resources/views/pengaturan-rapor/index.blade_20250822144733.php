<x-app-layout>
    ...
    <div class="max-w-xl mx-auto mt-8 rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        {{-- FORM HAPUS TTD (DIPISAH) --}}
        @if ($data?->ttd)
            <div class="mt-6 border-t pt-4">
                <p class="text-sm font-medium text-gray-700 dark:text-gray-400 mb-2">Tanda Tangan Kepala Sekolah</p>
                <img src="{{ asset('storage/' . $data->ttd) }}" alt="TTD Kepala Sekolah" class="h-16 mb-2">
                <form action="{{ role_route('pengaturan-rapor.hapus-ttd') }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus tanda tangan?');">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="text-sm text-red-600 hover:underline">Hapus Tanda Tangan</button>
                </form>
            </div>
        @endif
    </div>
