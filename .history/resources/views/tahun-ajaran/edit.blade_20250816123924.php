@foreach ($tahunAjaran as $item)
    <x-modal name="edit-modal-{{ $item['id'] }}" title="Edit Tahun Pelajaran" maxWidth="2xl">
        <form action="{{ role_route('tahun-ajaran.update', ['tahun_ajaran' => $item['id']]) }}" method="POST" class="space-y-6 sm:p-6">
            ...
            {{-- Mulai --}}
            {{-- <x-form.date-picker name="tahun_mulai" label="Tanggal Mulai" type="date" :value="old('tahun_mulai', $item['tahun_mulai'])" required /> --}}
<x-form.date-picker name="mulai" label="Tanggal Mulai" :value="old('mulai', $item['mulai'])" required />
            {{-- Selesai --}}
            {{-- <x-form.date-picker name="tahun_selesai" label="Tanggal Selesai" type="date" :value="old('tahun_selesai', $item['tahun_selesai'])" required /> --}}
<x-form.date-picker name="selesai" label="Tanggal Selesai" :value="old('selesai', $item['selesai'])" required />
            {{-- Status --}}
