<x-modal name="form-create-tahun" title="Tambah Tahun Pelajaran & Semester" maxWidth="2xl">
    <form action="{{ role_route('tahun-ajaran.store') }}" method="POST" enctype="multipart/form-data"
        ...
        {{-- Mulai --}}
        <x-form.date-picker name="mulai" label="Tanggal Mulai" required />

        {{-- Selesai --}}
        <x-form.date-picker name="selesai" label="Tanggal Selesai" required />
        ...
        <div x-data="{ isActive: {{ old('is_active') ? 'true' : 'false' }} }" @click.stop @mousedown.stop>
            ...
            <label for="is_active_create"
                ...
                Aktifkan Tahun Ajaran ini <span class="text-error-500">*</span>
            </label>
            @error('is_active')
