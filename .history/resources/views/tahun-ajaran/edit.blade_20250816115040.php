@foreach ($tahunAjaran as $item)
    <x-modal name="edit-modal-{{ $item['id'] }}" title="Edit Tahun Pelajatan & Semester" maxWidth="2xl">
        <form action="{{ role_route('tahun-ajaran.update', ['tahun_semester' => $item['id']]) }}" method="POST" class="space-y-6 sm:p-6">
            @csrf
            @method('PUT')
            {{-- Tahun --}}
            ...
            {{-- Mulai --}}
            <x-form.input name="tahun_mulai" label="Tanggal Mulai" type="date" :value="old('tahun_mulai', $item['tahun_mulai'])" required />
