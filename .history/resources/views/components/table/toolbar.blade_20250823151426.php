    ...
    <div
        ...
        <div
            ...
            <form method="GET" action="{{ $route }}">
                @if(request()->has('tahun_semester_filter'))
                    <input type="hidden" name="tahun_semester_filter" value="{{ request('tahun_semester_filter') }}">
                @endif
                @foreach ($filters as $filter)
                    <x-select-filter :name="$filter['name']" :label="$filter['label']" :options="$filter['options']"
                        :value-key="$filter['valueKey'] ?? 'id'"
                        :label-key="$filter['labelKey'] ?? 'name'"
                        :enabled="$filter['enabled'] ?? true"
                        :value="$filter['value'] ?? ''" />
