...
<x-table.toolbar {{-- :filters="$filters" --}} :enable-add-button="true" :enable-import="false" :enable-export="false" :enable-search="true"
    ...
    <x-slot name="addButton">
        <button type="button"
            onclick="window.dispatchEvent(new CustomEvent('open-modal', { detail: 'form-create-semester' }))"
...
<x-table :columns="[
    ...
    // 'semester_mulai' => ['label' => 'Tanggal Mulai', 'sortable' => true],
    // 'semester_selesai' => ['label' => 'Tanggal Selesai', 'sortable' => true],
    'status' => ['label' => 'Status', 'sortable' => false],
    // 'siswa_count' => ['label' => 'Jumlah Siswa', 'sortable' => false],
]" :data="$tahunSemester" :total-count="$tahunSemesterTotal" row-view="tahun-semester.partials.row"
