                                            ...
                                            <tr>
                                                ...
                                                @if ($tpList->isNotEmpty())
                                                    @foreach ($tpList as $tp)
                                                        ...
                                                        @foreach ($tpList as $tp)
    @php
        $val = old("nilai.{$gk->mapel->id}.{$ksId}.formatif_{$tp->id}") ?? ($nilaiMapelTab['formatif'][$ksId][$tp->id] ?? null);
    @endphp
    <td>
        <input x-show="editMode" type="number"
                                                ...
                                                @if ($lmList->isNotEmpty())
                                                    @foreach ($lmList as $bab)
                                                        ...
                                                        <td>
                                                         ...
                                                         @php
        $babKey = is_object($bab) ? $bab->id : $bab;
        $val = old("nilai.{$gk->mapel->id}.{$ksId}.sumatif_{$babKey}") ?? ($nilaiMapelTab['sumatif'][$ksId][$babKey] ?? null);
    @endphp
    <td>
        <input x-show="editMode" type="number"
