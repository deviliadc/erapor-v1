    ...
    <div x-data="{ editMode: false, activeKelas: '{{ $kelasId }}' }" class="bg-white dark:bg-white/[0.03] p-4 rounded-2xl">
        ...
        <form method="GET" class="mb-4 flex flex-wrap gap-4">
            ...
            <div class="flex-1 min-w-0">
                <x-form.select label="Tahun Semester" name="tahun_semester_id" :options="$tahunSemesterList->mapWithKeys(
                    fn($ts) => [
            ...
            {{-- Proyek --}}
            <div class="flex-1 min-w-0">
                <x-form.select label="Proyek" name="proyek_id" :options="$proyekList->mapWithKeys(fn($p) => [$p->id => $p->nama_proyek])" :selected="$proyekId"
                    placeholder="Pilih Proyek" searchable required onchange="this.form.submit()" />
            </div>
        </form>
                        ...
                        <div class="w-full overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
                            <table class="min-w-[1200px] w-full text-sm text-center table-auto whitespace-nowrap">
                                <thead class="text-gray-700 bg-gray-100 dark:bg-gray-800 dark:text-gray-200">
                                    <tr>
                                        <th class="px-3 py-2 text-left align-middle bg-gray dark:bg-gray-900"
                                            rowspan="2">No</th>
                                        <th class="px-3 py-2 text-left align-middle bg-gray dark:bg-gray-900"
                                            rowspan="2">Nama</th>
                                        <th class="px-3 py-2 text-left align-middle bg-gray dark:bg-gray-900"
                                        ...
                                        @foreach ($dimensiList as $dimensi)
                                            <th class="px-3 py-2 text-center bg-gray dark:bg-gray-900"
                                                colspan="{{ max($subelemenByDimensi[$dimensi->id]->count(), 1) }}">
                                        ...
                                        @foreach ($dimensiList as $dimensi)
                                            @if ($subelemenByDimensi[$dimensi->id]->isNotEmpty())
                                                @foreach ($subelemenByDimensi[$dimensi->id] as $subelemen)
                                                    ...
                                                    @endphp
                                                    <th
                                                        class="px-3 py-2 text-center text-xs font-normal italic max-w-xs truncate">
