<x-modal name="import-presensi-harian" title="Import Data Presensi Harian">
    <div class="p-4 space-y-4">
        ...
        {{-- <div class="flex items-center gap-5 mb-4">
            <select id="template-kelas-id" class="form-select w-48">
                <option value="">-- Pilih Kelas --</option>
                @foreach ($kelasList as $kelas)
                    <option value="{{ $kelas->id }}">{{ $kelas->nama }}</option>
                @endforeach
            </select>
            <a href="#" id="download-template-btn"
                class="inline-flex items-center gap-2 rounded-lg bg-white px-4 py-3 text-sm font-medium text-gray-700 shadow-theme-xs ring-1 ring-inset ring-gray-300 transition hover:bg-gray-50 dark:bg-gray-800 dark:text-gray-400 dark:ring-gray-700 dark:hover:bg-white/[0.03]">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round"
                        d="M4 16v2a2 2 0 002 2h12a2 2 0 002-2v-2M7 10l5 5m0 0l5-5m-5 5V4" />
        ...
        <script>
            document.getElementById('download-template-btn').addEventListener('click', function(e) {
                e.preventDefault();
                const kelasId = document.getElementById('template-kelas-id').value;
                if (!kelasId) {
                    alert('Pilih kelas terlebih dahulu!');
                    return;
                }
                window.open("{{ role_route('presensi-harian.template', ['kelas' => 'KELAS_ID']) }}".replace('KELAS_ID', kelasId), '_blank');
            });
        </script> --}}

                <div class="flex items-center gap-5 mb-4">
            <select id="template-bulan" class="form-select w-32">
                <option value="">-- Pilih Bulan --</option>
                @for ($m = 1; $m <= 12; $m++)
                    <option value="{{ $m }}">{{ \Carbon\Carbon::create()->month($m)->locale('id')->monthName }}</option>
                @endfor
            </select>
            <select id="template-tahun" class="form-select w-32">
        ...
        <script>
            document.getElementById('download-template-btn').addEventListener('click', function(e) {
                e.preventDefault();
                const bulan = document.getElementById('template-bulan').value;
                const tahun = document.getElementById('template-tahun').value;
                if (!bulan || !tahun) {
                    alert('Pilih bulan dan tahun terlebih dahulu!');
                    return;
                }
                window.open("{{ role_route('presensi-harian.template-bulan', ['bulan' => 'BULAN', 'tahun' => 'TAHUN']) }}"
                    .replace('BULAN', bulan).replace('TAHUN', tahun), '_blank');
            });
        </script>

        {{-- Form Upload --}}
        <form id="import-dropzone" action="{{ role_route('presensi-harian.import') }}" method="POST"
    ...
    alert(response.message || "Import berhasil.");
    window.dispatchEvent(new CustomEvent('close-modal', {
        detail: { name: 'import-presensi-harian' }
    }));
    window.location.reload();
