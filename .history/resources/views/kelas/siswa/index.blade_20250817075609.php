    ...
    <x-modal name="form-generate-absen" title="Generate Nomor Absen" maxWidth="md">
        <form
            action="{{ role_route('kelas.generate.absen', [
                'kelas' => $kelas->id,
                'tahun_semester_filter' => request('tahun_semester_filter') ?? ($tahunSemesterId ?? $tahunAjaranAktif?->id),
            ]) }}"
            method="GET">
            @csrf
            <input type="hidden" name="tahun_semester_filter" value="{{ request('tahun_semester_filter') ?? ($tahunSemesterId ?? $tahunAjaranAktif?->id) }}">
            <div class="p-6">
