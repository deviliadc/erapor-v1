...
<td class="px-4 py-3 text-gray-700 dark:text-gray-400 whitespace-nowrap min-w-[150px]">
    ...
    {{-- <a href="{{ role_route('kelas-siswa.show', ['kelas_siswa' => $item['id']]) }}"
        class="text-brand-500 hover:underline">
        {{ $item['siswa_count'] }} Siswa
    </a> --}}
    {{-- kelas/partials/row.blade.php --}}
    <a href="{{ role_route('kelas-siswa.show', [
        'kelas_siswa' => $item['id'],
        // 'tahun_ajaran_filter' => request('tahun_ajaran_filter') ?? ($tahunAjaranAktif->id ?? null)
        // 'tahun_ajaran_filter' => request('tahun_ajaran_filter') ?? ($tahunAjaranId ?? $tahunAjaranAktif?->id),
                'tahun_ajaran_filter' => request('tahun_ajaran_filter') ?? $tahunAjaranSelect,
    ]) }}"
        class="text-brand-500 hover:underline">
        {{ $item['siswa_count'] }} Siswa
    </a>
