                ...
                @forelse($siswaList as $i => $siswa)
                    <tr>
                        ...
                        <td class="px-3 py-2 flex flex-wrap gap-2 justify-center">
                            ...
                            if@canClick($isValidUTS)
                            <a href="{{ role_route('rapor.cetakTengah', ['siswa' => $siswa->id, 'tahun_semester_id' => $tahunSemesterId]) }}"
                                target="_blank"
                                class="rounded-lg px-4 py-3 text-sm font-medium shadow-theme-xs bg-brand-500 text-white hover:bg-brand-600">
                                Cetak Rapor UTS
                            </a>
                        @else
                            <span
                                class="rounded-lg px-4 py-3 text-sm font-medium shadow-theme-xs bg-gray-300 text-gray-500 cursor-not-allowed">
                                Cetak Rapor UTS
                            </span>
                            @endcanClick
                            ...
                            @if ($isValidP5)
                                <a href="{{ role_route('rapor.cetakP5', ['siswa' => $siswa->id, 'tahun_semester_id' => $tahunSemesterId]) }}"
                                    target="_blank"
                                    class="rounded-lg px-4 py-3 text-sm font-medium shadow-theme-xs bg-success-500 text-white hover:bg-success-600">
                                    Rapor P5
                                </a>
                            @else
                                <span
