@foreach ($guru as $item)
    <x-modal name="edit-modal-{{ $item['id'] }}" title="Edit Guru" maxWidth="2xl">
        <form action="{{ role_route('guru.update', ['guru' => $item['id']]) }}" method="POST"
            ...
            {{-- NUPTK --}}
            <x-form.input name="nuptk" label="NUPTK" placeholder="NUPTK" :value="old('nuptk', $item['nuptk'])" required />
            
            {{-- NIP --}}
            <x-form.input name="nip" label="NIP" placeholder="NIP" :value="old('nip', $item['nip'])" required />
