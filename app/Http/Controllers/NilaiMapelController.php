...
class NilaiMapelController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $periode = $request->input('periode', 'tengah');
        $daftarTahunSemester = $user->hasRole('guru') && $guru
            ? TahunSemester::whereIn('tahun_ajaran_id', GuruKelas::where('guru_id', $guru->id)->pluck('tahun_ajaran_id'))
            ->orderByDesc('id')->get()
            : TahunSemester::orderByDesc('id')->get();
        $tahunAktif = TahunSemester::find($request->tahun_semester_id)
            ?? $daftarTahunSemester->firstWhere('is_active', true)
        ...
        if ($kelasDipilih && $tahunAktif && $tahunAjaranId) {
            ...
            // }
            foreach ($mapel as $gk) {
                $lmList = LingkupMateri::with(['bab', 'tujuanPembelajaran' => function ($q) {
                ...
                $tpList = $lmList->flatMap->tujuanPembelajaran;
                $tujuanPembelajaranList[$gk->mapel_id] = $tpList;
                // Kirim ke blade hanya nama bab dari relasi
                $lingkupMateriList[$gk->mapel_id] = $lmList->map(function($lm) {
                    return (object)[
                        'id' => $lm->bab?->id,
                        'nama' => $lm->bab?->nama ?? '-' // nama bab, fallback '-'
                    ];
                });
            }
        ...
        foreach ($request->nilai ?? [] as $mapelId => $nilaiSiswa) {
            foreach ($nilaiSiswa as $kelasSiswaId => $nilaiSet) {
                foreach ($nilaiSet as $key => $nilai) {
                    if ($nilai !== null && $nilai !== '' && (float)$nilai < 0 || (float)$nilai > 100) {
                        return back()->with('error', 'Nilai harus di antara 0 sampai 100!');
                    }
                }
            }
        }

        // CEK VALIDASI SEMESTER
        $isValidated = \App\Models\ValidasiSemester::where('tahun_semester_id', $request->tahun_semester_id)
            ->where('tipe', $request->periode === 'tengah' ? 'UTS' : 'UAS')
            ->where('is_validated', true)
            ->exists();

        if ($isValidated) {
            return back()->with('error', 'Nilai tidak dapat diubah karena semester sudah divalidasi.');
        }
        DB::beginTransaction();
                    ...
                    if ($nilaiTerisi->isEmpty()) {
                        ...
                        if ($nilaiMapel) {
                            foreach ($nilaiSet as $key => $nilai) {
                                ...
                                NilaiMapelDetail::where('nilai_mapel_id', $nilaiMapel->id)
                                    ->where('jenis_nilai', $data['jenis_nilai'])
                                    ->when($data['tujuan_pembelajaran_id'], fn($q, $v) => $q->where('tujuan_pembelajaran_id', $v))
                                    ->when($data['lingkup_materi_id'], fn($q, $v) => $q->where('lingkup_materi_id', $v))
                                    ->delete();
                            }
