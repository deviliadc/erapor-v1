...
class SiswaController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $columnMap = [
            ...
            'nama_ayah' => 'waliMurid.nama_ayah',
            'nama_ibu' => 'waliMurid.nama_ibu',
            'nama_wali' => 'waliMurid.nama_wali',
            'pendidikan_sebelumnya' => 'siswa.pendidikan_sebelumnya',
            'alamat' => 'siswa.alamat',
            'no_hp' => 'siswa.no_hp',
            'no_hp_wali' => 'waliMurid.no_hp',
            'email' => 'users.email',
            'status' => 'siswa.status'
        ];
        $query = Siswa::query()
        ...
        if ($request->filled('search')) {
            ...
            $query->where('nama', 'like', "%{$q}%")
                ...
                ->orWhere('wali.nama_ayah', 'like', "%{$q}%")
                ->orWhere('wali.nama_ibu', 'like', "%{$q}%")
                ->orWhere('wali.nama_wali', 'like', "%{$q}%");
        }
        ...
        $siswa = $paginator->through(fn($item) => [
            ...
            'nama_ayah' => $item->waliMurid?->nama_ayah ?? '-',
            'nama_ibu' => $item->waliMurid?->nama_ibu ?? '-',
            'nama_wali' => $item->waliMurid?->nama_wali ?? '-',
            'pendidikan_sebelumnya' => $item->pendidikan_sebelumnya ?? '-',
            'alamat' => $item->alamat ?? '-',
            'no_hp' => $item->no_hp ?? '-',
            'no_hp_wali' => $item->waliMurid?->no_hp ?? '-',
        ...
        $title = 'Manage Siswa';
        return view('siswa.index', compact('siswa', 'totalCount', 'breadcrumbs', 'title'));
    }
    ...
    public function template()
    {
        ...
        // Export hanya header sebagai template
        return Excel::download(
            new ReusableExport($header, []),
            'template_siswa.xlsx'
        );
    }
    ...
    // public function template()
    // {
    //     return Storage::download('template/template_siswa.xlsx');
    // }
    ...
    public function import(Request $request)
    {
        ...
        return redirect()->to(role_route('siswa.index'))->with('success', 'Import data siswa berhasil.');
    }
}
