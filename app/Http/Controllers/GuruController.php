            ...
            new ReusableImport(function ($row, $index) use (&$success, &$failed, &$errors, &$headerError, $requiredHeaders) {
                ...
                if ($index === 0) {
                    if ($dataArr !== $requiredHeaders) {
                        $headerError = true;
                        $failed++;
                        $errors[] = 'Header file tidak sesuai.';
                        return;
                    }
                    return;
                }
                ...
                $data['NIP'] = isset($data['NIP']) ? (string) $data['NIP'] : null;
                $data['NUPTK'] = isset($data['NUPTK']) ? (string) $data['NUPTK'] : null;
if ($data['NIP'] && str_starts_with($data['NIP'], "'")) {
    $data['NIP'] = ltrim($data['NIP'], "'");
}
if ($data['NUPTK'] && str_starts_with($data['NUPTK'], "'")) {
    $data['NUPTK'] = ltrim($data['NUPTK'], "'");
}
                ...
                try {
                    ...
                    $failed++;
                    $errors[] = ($data['Nama'] ?? '(Tanpa Nama)') . ' gagal disimpan: ' . $e->getMessage();
                }
            }),
            $request->file('file')
        );
        ...
        if ($failed > 0 || $success === 0) {
            ...
            return redirect()->to(role_route('guru.index'))->with('error', $msg);
        }
