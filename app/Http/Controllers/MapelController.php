...
if ($request->has('search_tujuan_pembelajaran') && $request->input('tab') !== 'tujuan-pembelajaran') {
    ...
    public function update(Request $request, string $id)
    {
        ...
        // data nggaboleh sama
        if (Mapel::where('kode_mapel', $validated['kode_mapel'])->where('id', '!=', $mapel->id)->exists()) {
            return redirect()->back()->withErrors(['kode_mapel' => 'Kode mapel sudah digunakan.'])->withInput();
        }

        // urutan ngga boleh sama
        if (Mapel::where('urutan', $validated['urutan'])->where('id', '!=', $mapel->id)->exists()) {
            return redirect()->back()->withErrors(['urutan' => 'Urutan sudah digunakan.'])->withInput();
        }
